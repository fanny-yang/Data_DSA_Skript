...
Beispiel für die Lösung eines solchen Minimierungsproblems wäre die Methode der kleinsten Quadrate. Gegeben ist hierbei eine lineare Funktion $f$ und Punkte $P_i=(x_i,y_i)$, welche näherungsweise auf der Geraden liegen \ref{fig:LinReg}:
...
$f(a^{t+1}) < f(a^{t})  \qquad \mid \partial f(a^{(t)}) \neq 0$ \\
$g^{+}(\lambda)=f(a^+-\lambda\nabla f(a^t))$ \\
...
Für $f: D\rightarrow \mathbb{R}$ ist $x\in D$ ein lokales Minimum, wenn mindestens eine Umgebung $N$ existiert, sodass \\$\forall_{y\in N}$ gilt: $f(y)\geq f(x)$ \\
...
\end{align*}
Dies kann dank der Dreiecksungleichung und der Ausnutzung der Linearität der Norm gezeigt werden.