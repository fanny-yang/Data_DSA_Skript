...
Um die Anzahl an Parametern zu vergrößern, werden diese auch über nicht-lineare Funktionen miteinander verknüpft (die Verkettung von linearen Funktion wäre wieder linear und besäße deshalb nur begrenzte Representationskraft). Jedoch kann eine zu hohe Anzahl an Parametern auch dazu führen, dass das Netzwerk \enquote{overfittet}, das heißt zu sehr an den Trainingsdatensatz angepasst wird. Dann kann das Netzwerk selbst geringe Abweichungen von den Trainingsdaten nicht mehr korrekt klassifizieren.
