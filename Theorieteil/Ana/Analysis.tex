...
Falls mehrere Funktionen miteinander verknüpft sind gelten folgende drei Regeln.
\begin{itemize}
...
\begin{equation*}
\begin{split} h(x+v) & = f(x) \cdot g(x)  + f(x)  \cdot D_{g}(x) \cdot v \\ & + f(x) \cdot r_{g}(v) + Df(x) \cdot v \cdot g(x) \\ & +  Df(x) \cdot v \cdot D_{g}(x) \cdot v  + Df(x) \cdot v \cdot r_{g}(v) \\ & +  r_{f}(v) \cdot g(x) + r_{f}(x) \cdot Dg(x) \cdot v \\ & + r_{f}(v) \cdot r_{g}(v) \end{split} \end{equation*}   %Die Formel passt nicht in eine Zeile. Umbruch?
